  ...
  return (
    <div className="rtm-fade-in" style={{ maxWidth: 880, margin: '0 auto' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '18px' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', fontSize: '10px', fontWeight: 800, letterSpacing: '.6px', textTransform: 'uppercase', color: '#06121d', background: 'var(--rtm-gold)', padding: '4px 12px', borderRadius: '999px' }}>★ {ct('Conta vitalícia · apoiador')}</span>
        <h1 style={{ margin: '10px 0 0', fontFamily: 'var(--rtm-font-cond)', fontSize: '32px', fontWeight: 800, textTransform: 'uppercase', color: 'var(--rtm-text-strong)' }}>{ct('Suas carreiras')}</h1>
        <p style={{ color: 'var(--rtm-dim)', fontSize: '14px', maxWidth: '520px', margin: '8px auto 0', lineHeight: 1.55 }}>
          {ct('Você pode manter até')} {CAREER_SLOTS} {ct('carreiras salvas ao mesmo tempo e apagar qualquer uma quando quiser para começar outra.')} <b style={{ color: 'var(--rtm-text)' }}>{used}/{CAREER_SLOTS}</b> {ct('em uso')}{loadingCloud ? ` · ${ct('sincronizando com a nuvem…')}` : ''}.
        </p>
      </div>
            ...
            return (
              <div key={s.slot} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px 18px', borderRadius: '10px', background: 'var(--rtm-bg-deep)', border: '1px dashed var(--rtm-border)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: 'var(--rtm-font-cond)', fontWeight: 700, fontSize: '17px', color: 'var(--rtm-dim)' }}>{ct('Slot')} {s.slot} · {ct('vazio')}</div>
                  <div style={{ fontSize: '12px', color: 'var(--rtm-faint)' }}>{ct('Comece uma carreira nova do zero.')}</div>
                </div>
                <Button variant="primary" disabled={busySlot != null} onClick={() => void go(s.slot)}>{ct('Novo save')}</Button>
              </div>
            );
          }
          return (
            <div key={s.slot} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px 18px', borderRadius: '10px', background: 'var(--rtm-panel)', border: '1px solid var(--rtm-border-soft)' }}>
              ...
              {confirmSlot === s.slot ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ fontSize: '12px', color: 'var(--rtm-red-bright)', fontWeight: 700 }}>{ct('Apagar?')}</span>
                  <Button variant="danger" size="sm" onClick={() => doDelete(s.slot)}>{ct('Sim, apagar')}</Button>
                  <Button variant="ghost" size="sm" onClick={() => setConfirmSlot(null)}>{ct('Não')}</Button>
                </div>
              ) : (
                <div style={{ display: 'flex', gap: '8px' }}>
                  <Button variant="gold" disabled={busySlot != null} onClick={() => void go(s.slot, s.fromCloud)}>{busy ? ct('Carregando…') : ct('Continuar')}</Button>
                  <Button variant="ghost" size="sm" disabled={busySlot != null} onClick={() => setConfirmSlot(s.slot)}>{ct('Apagar')}</Button>
                </div>
              )}
            </div>
          );
      ...
      <Panel title={ct('Como funciona')} accent="blue" style={{ marginTop: '18px' }}>
        <p style={{ margin: 0, fontSize: '13px', color: 'var(--rtm-dim)', lineHeight: 1.55 }}>
          {ct('Cada save é uma carreira independente (org, elenco, títulos, dinheiro). Apagar um save libera o slot na hora e não dá pra desfazer. Como sua conta é vitalícia, seus saves sincronizam na nuvem e aparecem em qualquer aparelho.')}
        </p>
      </Panel>
    </div>
  );
