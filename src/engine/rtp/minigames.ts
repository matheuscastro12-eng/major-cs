...
// Cada ação treinável → um minijogo. rest/social/stream ficam INSTANTÂNEOS
// (jogar um puzzle pra "descansar" seria anti-fun).
export const ACTION_GAME: Partial<Record<ActionKind, MiniGameId>> = {
  'train:mechanical': 'flick',
...
export function gameForAction(kind: ActionKind): MiniGameDef | null {
  const id = ACTION_GAME[kind];
  return id ? MINIGAMES[id] : null;
}
